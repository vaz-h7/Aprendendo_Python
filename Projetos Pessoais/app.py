...
# --- FUNÇÃO PARA CARREGAR DADOS (MÉTODO CSV PÚBLICO) ---
@st.cache_data(ttl=60)
def load_data():
    # ID da sua planilha extraído da URL
    SHEET_ID = "1pkDTOC38D5rf1BbCBAAAOC3qEMVa6y-8hsy_KH_A2x0"
    ...
    if 'Data' in df.columns:
        ...
        # Novas colunas para facilitar filtros
...
try:
    ...
    if df.empty:
        st.warning("Nenhum dado encontrado ou planilha está privada. Verifique o compartilhamento.")
    else:
        ...
        df_meses = df_ano_filtrado[['Mes_Ano_Exibicao', 'Mes_Ano']].drop_duplicates().sort_values('Mes_Ano',
                                                                                                  ascending=False)
        ...
        with c1:
            ...
            if not df_pizza.empty:
                st.plotly_chart(px.pie(df_pizza, values="Valor", names="Categoria", hole=0.4), use_container_width=True)
        with c2:
            st.subheader("Balanço Mensal")
            df_balanco = pd.DataFrame({'Status': ['Receitas', 'Despesas'], 'Total': [Receitas_total, saidas_total_abs]})
            st.plotly_chart(px.bar(df_balanco, x='Status', y='Total', color='Status',
                                   color_discrete_map={"Receitas": "#2ecc71", "Despesas": "#e74c3c"}),
